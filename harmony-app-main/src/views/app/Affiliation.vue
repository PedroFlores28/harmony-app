<template>
  <App :session="session" :office_id="office_id" :title="title">
    <div v-cloak>
      ...
      <MigrationBlockedModal
        v-if="!loading && showMigrationBlockedModal && !(affiliation && affiliation.status === 'pending')"
        :plan-name="userPlanDisplayName"
        @close="goDashboardFromMigrationBlock"
      />
        ...
        <section v-if="!loading && !showMigrationBlockedModal && !(affiliation && affiliation.status === 'pending')" class="affiliation-main-container">
...
export default {
  ...
  data() {
    return {
      ...
      showMigrationBlockedModal: false,
  ...
  computed: {
    ...
    userPlanDisplayName() {
      const planId =
        ...
        (this.affiliation && this.affiliation.plan && this.affiliation.plan.id);
      return planDisplayName(planId, this.plans);
    },
  },
  watch: {
    selec_plan() {
  ...
  async created() {
    try {
      ...
      this.applyMigrationBlockFromApi(data);
  ...
  methods: {
    applyMigrationBlockFromApi(data) {
      this.showMigrationBlockedModal = Boolean(data && data.migrationBlocked);
    },
    goDashboardFromMigrationBlock() {
    ...
    async POST() {
      ...
      if (data.error) {
        if (data.msg && String(data.msg).toLowerCase().includes("platino")) {
          this.showMigrationBlockedModal = true;
          return;
        }
        this.error = data.msg || "No se pudo procesar la solicitud";
        return;
      }
