...
<div v-if="children && children.length && children_points && children_points.length">
  ...
  <div class="modal" :class="{ open }" @click="closed">
    <div class="inner" @click.stop="">
      <img class="photo" :src="selec_node.photo" style="display:block; margin:auto; border-radius:50%; width:100px; height:100px; object-fit:cover; border:3px solid #0476D9;">
      ...
      <div v-if="modal_children && modal_children.length && modal_children_points && modal_children_points.length">
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 10px; justify-content: center;">
          <div v-for="(child, idx) in modal_children" :key="child.id" style="background: rgba(4, 118, 217, 0.1); border-radius: 12px; box-shadow: 0 2px 8px #0476D920; padding: 12px 18px; min-width: 180px; display: flex; align-items: center; gap: 10px;">
            <i class="fas fa-user-circle" style="font-size: 28px; color: #0476D9;"></i>
            <div style="flex:1;">
              <div style="font-weight: bold; color: #333; font-size: 15px;">{{ child.name }}</div>
              <div style="color: #888; font-size: 13px;">Puntos grupales: <span style="color: #0476D9; font-weight: bold;">{{ modal_children_points[idx] }}</span></div>
            </div>
          </div>
        </div>
      </div>
...
<style>
.selected-node > span {
  box-shadow: 0 0 8px #0476D9 !important;
